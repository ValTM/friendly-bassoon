...
function Ecommerce() {
  ...
  const openNav = () => {
    ...
    overlay.classList.add('w-screen', 'h-screen', 'opacity-85');
  };

  const hideNav = () => {
    !showNav(!nav);
    overlay.classList.remove('w-screen', 'h-screen', 'opactiy-95');
    ...
    <div>
      ...
      <div className="ec--body ">
        <nav className="relative flex justify-between items-center h-[60px]  px-5">
          <div className="flex items-center justify-between w-[150px] z-10   ">
            <img onClick={openNav} src={hamburger} alt="" />
            <img className="ml-2" src={logo} alt="" />
          </div>
          {nav && (
            <div className="absolute z-[11] top-0  left-0 w-[70%] h-screen bg-white ">
              <img
          ...
          <div
            id="overlay"
            className="absolute top-0 left-0 w-0 h-0 bg-black z-10   "
          ></div>
        </nav>
      </div>
    </div>
